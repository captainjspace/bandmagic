...
const colors = {
  ...
  newLink:
    "border border-neutral-700 hover:border-green-600 text-neutral-200 hover:text-green-400 rounded px-3 py-1.5 text-xs transition-colors",
};
  ...
  return (
    <div>
      <div className="mb-10 flex items-start justify-between">
        <div>
          <h1
            className={`text-2xl font-bold ${colors.page.title} tracking-tight`}
          >
            TrackGroups
          </h1>
          <p className={`${colors.page.count} text-sm mt-1`}>
      ...
      <div className="space-y-4">
        {trackGroups.map((trackGroup) => (
          <div
            key={trackGroup.id}
            className="border border-neutral-800 rounded-lg hover:border-neutral-600 hover:bg-neutral-900 transition-all group"
          >
            <div className="flex items-start gap-4">
              ...
              >
                <h2
                  className={`${colors.trackGroupCard.title} font-semibold transition-colors truncate`}
                >
                  {trackGroup.title}
                </h2>
                {trackGroup.description && (
                  <p
                    className={`${colors.trackGroupCard.description} text-sm mt-1 line-clamp-2`}
                  >
                    {trackGroup.description}
                  </p>
                )}
