...
export function reconcileAssetLinks(existing: AssetLink[] | undefined, nextIds: string[], addedBy: string): AssetLink[] {
  const byAssetId = new Map((existing ?? []).map(l => [l.assetId, l]));
  const now = new Date().toISOString();
  return nextIds.map(assetId => byAssetId.get(assetId) ?? { linkId: uid(), assetId, addedAt: now, addedBy });
}
...
export function assetLocationMap(
  ...
  const add = (assetId: string, label: string) => locations.set(assetId, [...(locations.get(assetId) ?? []), label]);
  for (const link of groupAssets ?? []) add(link.assetId, 'Track Group');
  for (const t of tracks) for (const link of t.assets ?? []) add(link.assetId, `Track: ${t.title || 'untitled'}`);
  return new Map([...locations].map(([assetId, labels]) => [assetId, labels.join(', ')]));
}
