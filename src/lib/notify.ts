...
async function sendEmail(release: Release) {
  ...
  const body = [
    ...
    tracklist,
    '',
    `Listen: ${process.env.APP_URL ?? 'https://rollingblackoutband.com'}/release/${release.id}`,
  ].join('\n');
...
async function sendChatMessage(release: Release) {
  const text = `🎵 *New release: ${release.title}*\n${release.description ?? ''}\n${release.tracks.length} tracks — ${process.env.APP_URL ?? 'https://rollingblackoutband.com'}/release/${release.id}`;
