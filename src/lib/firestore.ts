...
export async function createTrackGroup(trackGroup: Omit<TrackGroup, 'id'>): Promise<TrackGroup> {
...
export async function getNotes(trackGroupId: string, trackPath?: string): Promise<Note[]> {
  let query = db().collection(firestoreConfig.coredb).doc(trackGroupId).collection('notes').orderBy('createdAt', 'asc') as FirebaseFirestore.Query;
  if (trackPath) query = query.where('trackPath', '==', trackPath);
  const snap = await query.get();
  return snap.docs.map(d => ({ id: d.id, ...d.data() } as Note));
}

export async function addNote(trackGroupId: string, note: Omit<Note, 'id'>): Promise<Note> {
  const ref = await db().collection(firestoreConfig.coredb).doc(trackGroupId).collection('notes').add({
    ...note,
