...
cleanup() {
  ps -o pid= -o comm= -t $(tty) | perl -lane 'kill "TERM", $F[0] if $F[1] =~ /tee|tail|nc/'
...
exec 2> >(tee -a "$LOG_ERR" >&2) \
     1> >(tee -a "$LOG_OUT"  >&1) 
trap 'ps -ef | grep "tail -q -F $LOG_ERR" | grep -v grep | awk "{print $2}" | while read l;kill HUP $l;' EXIT
( tail -q -F "$LOG_ERR" "$LOG_OUT" | nc -U /tmp/deploy.sock ) &    
...
init_flags() {
  ...
  FLAGSET=(
    ...
    # accounts
    SERVICE_ACCOUNT_EXISTS DRIVE_SERVICE_EXISTS SA CONFIG_VALID 
    # resources 
    ARTIFACT_REPO_CREATED DOCKER_FORMAT REPO_LOCATION_CORRECT
    SECRETS_CREATED
    GIT_FEDERATION
    # access policy
    FIRESTORE_ACCESS CLOUD STORAGE_ACCESS DRIVE_SERVICE_TOKENS
    # federation
